...
table3Seattle <- rbind(sf(sASeattle[[1]][1:50,4:7]),
                  sf(sBSeattle[[1]][1:50,4:7]),
                  sf(sCSeattle[[1]][1:50,4:7]),
                  sf(sDSeattle[[1]][1:50,4:7]),
                  sf(sESeattle[[1]][1:50,4:7]),
                  sf(sFSeattle[[1]][1:50,4:7]),
                  sf(sGSeattle[[1]][1:50,4:7]),
                  sf(sHSeattle[[1]][1:50,4:7]))
write.csv(table3Seattle, 'table3Seattle.csv')
...
pf <- function(cases, ymax = 300, xoff = 0, yoff = 0, cols = brewer.pal(9, 'Greens')[c(3, 8)]) {
  tot <- rowSums(cases)
...
pdf('fig3Seattle.pdf', height = 10, width = 15)

plot(1, type = 'n', xlim = c(0, nd * 4), ylim = c(0, 300), axes = F, xlab = NA, ylab = NA)

rect(0, 0, nd * 4, 150, col = 'grey98')

pf(sASeattle[[2]][1:50,], yoff = 75)
pf(sBSeattle[[2]][1:50,])
pf(sCSeattle[[2]][1:50,], yoff = 75, xoff = nd)
pf(sDSeattle[[2]][1:50,], xoff = nd)
pf(sESeattle[[2]][1:50,], yoff = 75, xoff = nd * 2)
pf(sFSeattle[[2]][1:50,], xoff = nd * 2)
pf(sGSeattle[[2]][1:50,], yoff = 75, xoff = nd * 3)
pf(sHSeattle[[2]][1:50,], xoff = nd * 3)
