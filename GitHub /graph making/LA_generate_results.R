...
table3LA <- rbind(sf(sALA[[1]][1:50,4:7]),
                sf(sBLA[[1]][1:50,4:7]),
                sf(sCLA[[1]][1:50,4:7]),
                sf(sDLA[[1]][1:50,4:7]),
                sf(sELA[[1]][1:50,4:7]),
                sf(sFLA[[1]][1:50,4:7]),
                sf(sHLA[[1]][1:50,4:7]))
write.csv(table3LA, 'table3LA.csv')
...
pdf('fig3LA.pdf', height = 10, width = 15)

plot(1, type = 'n', xlim = c(0, nd * 4), ylim = c(0, 600), axes = F, xlab = NA, ylab = NA)

rect(0, 0, nd * 4, 600, col = 'grey98')

pf(sALA[[2]][1:50,], yoff = 300)
pf(sBLA[[2]][1:50,])
pf(sCLA[[2]][1:50,], yoff = 300, xoff = nd)
pf(sDLA[[2]][1:50,], xoff = nd)
pf(sELA[[2]][1:50,], yoff = 300, xoff = nd * 2)
pf(sFLA[[2]][1:50,], xoff = nd * 2)
pf(sHLA[[2]][1:50,], xoff = nd * 3)
...
text(rep(c(nd/2*0.05 + modelend/2, nd * 1 + modelend/2, nd * 2 + modelend/2, nd*3 + modelend/2), each = 2), c(590, 290, 590, 290, 590, 290),
     c('Scenario A\n(Base Case;\nday 61)',
       'Scenario B\n(No Hotel Housing)',
