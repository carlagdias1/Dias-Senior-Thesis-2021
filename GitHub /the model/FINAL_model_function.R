...
# 7 = HOTEL: susceptible
# 8 = HOTEL: exposed
# 9 = HOTEL: infectious pre-symptomatic
# 10 = HOTEL: infectious symptomatic (transferred to COVID-CARE next day)
# 11 = HOTEL: recovered

# 12 = QUARANTINE: infectious symptomatic
# 13 = QUARANTINE: not infectious symptomatic
...
load("TESTcluster_ids.RData")

# background incidence
# --------------------

background_incidence <- read.csv("Gen_Pop_Cases.csv", colClasses = c('Date', rep('integer', 6)))
background_incidence$date <- NULL
gp_scenarios <- c('DC', 'LA', 'SF', 'Seattle', 'Extra')
# city populations
# DC = 692683
# LA = 10040000
# SF = 874961
# Seattle = 724305
background_incidence[, gp_scenarios] <- background_incidence[, gp_scenarios] / 10040000 # population of city
...
run_model <- function(
  ...
  gen_pop_mix_end = 15, # period 1 end. set to NA for the whole duration to be as per 'gen_pop_mix_para2'
  gen_pop_mix_para = 0.5, # paramenter 'm' for period 1
  gen_pop_mix_para2 = 0.1, # parameter 'm' for period 2
  bi = 'LA', # relevant column header in object 'background_incidence'
  ...
  intervention_ends = NA, # set to NA for intervention does not end
  dur.PROTECT_rec = 30, # HOTEL population recruited steadily over this period (days)
  # DC = 0.8
  # LA = 0.4
  # SF = 0.6
  # Seattle = 0.7
  accept_ss = 0.4, # proportion encampments to HOTEL when offered
  accept_CARE = 0.8, # proportion accepting QUARANTINE when offered
  ...
  covid_severity = c(0.4, 0.54, 0.0359, 0.0241), # asymptomatic / mild / moderate / severe. should sum to 1
  cfr = c(0, 0.00796, 0.15, 0.45), # by severity
  ...
  f <- function (day) {
    ...
    q.CAR <- (day - day.sym.i) == (day_of_infection + 1) # offered QUARANTINE
    ...
    location <- findInterval(sy, c(0, 7, 12, 14)) # community, HOTEL, QUARANTINE, hospital/dead
    ...
    iN[location == 2] <- sum(sy == 9 | sy == 10) / sum(location == 2) # HOTEL
    
    # cluster-specific r0's
    r0i <- if (day %in% suppressDays) r0i_period1 else r0i_period2
    r0i <- ifelse(location == 2, r0i_cp, r0i) # set to relevant value for community cluster or HOTEL
    ...
    p.rCC <- (rbinom(n, 1, accept_CARE) == 1) & started & (severity != 1) # accept_ss QUARANTINE referral
    p.rCP <- (rbinom(n, 1, accept_ss) == 1) & started # encampments accept HOTEL referral
    p.rCP[type == 2] <- T
    p.rCP[type == 1] <- F
    ...
    status[sy == 10] <- 12L # transferred to QUARANTINE (even if asymptomatic to reflect testing)
